...
public static class BaseCatalogSeeder
{
    ...
    public static void SeedBaseCatalogs(this GanaderiaDbContext ganaderiaDb)
    {
        ...
        if (!ganaderiaDb.cat_razas.Any(x => x.code == RazaCode))
            ganaderiaDb.cat_razas.Add(new cat_raza
                { code = RazaCode, nombre = "Holstein", activo = true });

        if (!ganaderiaDb.cat_colors.Any(x => x.code == ColorCode))
            ganaderiaDb.cat_colors.Add(new cat_color
                { code = ColorCode, nombre = "Blanco", activo = true });
        ...
        if (!ganaderiaDb.cat_sexos.Any(x => x.code == SexoMachoCode))
        ...
        if (!ganaderiaDb.cat_tipo_adquisicions.Any(x => x.code == TipoAdquisicionCode))
            ganaderiaDb.cat_tipo_adquisicions.Add(new cat_tipo_adquisicion
                { code = TipoAdquisicionCode, nombre = "Nacimiento", activo = true });

        if (!ganaderiaDb.cat_tipo_adquisicions.Any(x => x.code == TipoAdquisicionCompraCode))
            ganaderiaDb.cat_tipo_adquisicions.Add(new cat_tipo_adquisicion
                { code = TipoAdquisicionCompraCode, nombre = "Compra", activo = true });

        if (!ganaderiaDb.cat_estado_vacunos.Any(x => x.code == EstadoVacunoCode))
            ganaderiaDb.cat_estado_vacunos.Add(new cat_estado_vacuno
                { code = EstadoVacunoCode, nombre = "Sano" });

        if (!ganaderiaDb.cat_estado_vacunos.Any(x => x.code == EstadoVacunoEliminadoCode))
            ganaderiaDb.cat_estado_vacunos.Add(new cat_estado_vacuno
                { code = EstadoVacunoEliminadoCode, nombre = "Muerto" });

        if (!ganaderiaDb.cat_tipo_utilizacions.Any(x => x.code == UtilizacionCode))
            ganaderiaDb.cat_tipo_utilizacions.Add(new cat_tipo_utilizacion
                { code = UtilizacionCode, nombre = "Produccion de leche", activo = true });
    }
}
