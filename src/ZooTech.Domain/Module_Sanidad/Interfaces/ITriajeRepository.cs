...
public interface ITriajeRepository
{
    ...
    Task<ResumenSanidad> GetResumenAsync(CancellationToken cancellationToken = default);
    Task<IEnumerable<PesoPromedioItem>> GetPesoPromedioPorPeriodoAsync(int meses, CancellationToken cancellationToken = default);
    Task<IEnumerable<TipoPesoCountItem>> GetDistribucionTipoPesoAsync(CancellationToken cancellationToken = default);
}
