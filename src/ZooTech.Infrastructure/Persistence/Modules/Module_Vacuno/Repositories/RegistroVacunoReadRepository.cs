...
public sealed class RegistroVacunoReadRepository : IRegistroVacunoReadRepository
{
    ...
    public async Task<string?> ObtenerNombreCatalogoAsync(string tipoCatalogo, string codigo, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(codigo)) return null;

        return tipoCatalogo.ToLowerInvariant() switch
        {
            "raza" => await _context.cat_razas.Where(x => x.code == codigo).Select(x => x.nombre).FirstOrDefaultAsync(cancellationToken),
            "sexo" => await _context.cat_sexos.Where(x => x.code == codigo).Select(x => x.nombre).FirstOrDefaultAsync(cancellationToken),
            "color" => await _context.cat_colors.Where(x => x.code == codigo).Select(x => x.nombre).FirstOrDefaultAsync(cancellationToken),
            "tipoadquisicion" => await _context.cat_tipo_adquisicions.Where(x => x.code == codigo).Select(x => x.nombre).FirstOrDefaultAsync(cancellationToken),
            _ => null
        };
    }

    public async Task<string?> ObtenerCodigoVacunoAsync(long id, CancellationToken cancellationToken = default)
    {
