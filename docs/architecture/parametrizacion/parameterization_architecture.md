...
### Diseño del T4 Fusionado

El template combina la **type-safety** del plan original (`SettingDefinition<T>`, `FeatureFlag`, `RuleSchema`) con las **mejoras operativas** del template alternativo (`COALESCE`, `ExtractJsonValue`, `FormatValue`, `ToPascalCase` robusto).
...
**Query 1 — Settings (con valor efectivo del tenant):**
```sql
SELECT
...
[1] Leer connection string de appsettings.Development.json
[2] try/catch global — si falla, generar stub vacío
[3] Query 1: Settings → agrupar por GroupCode → sub-clases en ZooSettings
[4] Query 2: Features → agrupar por category → sub-clases en ZooFeatures
[5] Query 3: Rules → agrupar por module → sub-clases en ZooRules
[6] Para cada setting:
    - PascalCase(SettingCode) → nombre del campo
    - MapToCSharpType(DataType) → tipo genérico T
    - ExtractJsonValue(FinalValue) → limpiar JSON envuelto
    - FormatValue(limpio, csharpType) → valor formateado para DefaultValue
    - Generar XML doc comment con código, tipo y descripción
[7] Para cada feature:
    - PascalCase(code) → nombre del campo
    - is_active → DefaultValue (bool)
[8] Para cada rule:
    - PascalCase(code) → nombre del campo
    - condition_schema, action_schema → parámetros de RuleSchema
```

#### Funciones helper (clase feature del T4)
...
#### Lectura de connection string desde `appsettings.Development.json`

```csharp
<#
    string appSettingsPath = Path.Combine(
        Path.GetDirectoryName(Host.ResolvePath("")),
        "..", "..", "src", "ZooTech.API", "appsettings.Development.json");

    string connectionString = "";
    if (File.Exists(appSettingsPath))
    {
        string json = File.ReadAllText(appSettingsPath);
        // Parse simple: buscar "TenantCatalogConnection": "..."
        var match = Regex.Match(json, "\"TenantCatalogConnection\"\\s*:\\s*\"([^\"]+)\"");
        if (match.Success)
...
#### Stub vacío en caso de error de conexión
...
public static class ZooRules { }
<#
        return; // o el equivalente en T4 para detener la generación
    }
#>
```
